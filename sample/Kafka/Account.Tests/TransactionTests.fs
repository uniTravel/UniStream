...
let test1 =
    [ testCase "初始化"
      <| fun _ ->
          ...
          let agg = svc.OpenPeriod id1 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = svc.TransferOut id1 com |> Async.RunSynchronously
...
let test2 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          ...
          let agg = svc.ChangeLimit id2 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.Withdraw id2 com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "转出金额超过余额"
      <| fun _ ->
          let com = TransferOut(Amount = 100m)
          let f = fun _ -> svc.TransferOut id2 com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "初始创建的交易期间"
    |> testSequenced
    |> testLabel "Transaction"
