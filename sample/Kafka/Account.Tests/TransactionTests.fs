...
let test1 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          ...
          let agg = svc.TransferOut None id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.SetTransLimit None id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "设置交易限额"
      <| fun _ ->
          let com = SetTransLimit(TransLimit = 500m)
          let agg = svc.SetTransLimit None id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = svc.ChangeLimit None id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.Withdraw None id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "转出金额超过余额"
      <| fun _ ->
          let com = TransferOut(Amount = 100m)
          let f = fun _ -> svc.TransferOut None id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "初始创建的交易期间"
    |> testSequenced
