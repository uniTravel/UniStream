...
let test1 =
    [ testCase "初始化"
      <| fun _ ->
          ...
          let agg = svc.CreateAccount None id1 com |> Async.RunSynchronously
          Expect.equal agg.Owner "张三" "聚合值有误"
          let com = VerifyAccount(VerifiedBy = "王五", Conclusion = true)
          let agg = svc.VerifyAccount None id1 com |> Async.RunSynchronously
          ...
          let agg = svc.ApproveAccount None id1 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = svc.LimitAccount None id1 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          let com = LimitAccount(Limit = 20m)
          let f = fun _ -> svc.LimitAccount None id1 com |> Async.RunSynchronously |> ignore
...
let test2 =
    [ testCase "初始化"
      <| fun _ ->
          let com = CreateAccount(Owner = "张三")
          let agg = svc.CreateAccount None id2 com |> Async.RunSynchronously
          Expect.equal agg.Owner "张三" "聚合值有误"
          let com = VerifyAccount(VerifiedBy = "王五", Conclusion = true)
          let agg = svc.VerifyAccount None id2 com |> Async.RunSynchronously
          Expect.equal (agg.Owner, agg.VerifiedBy, agg.Verified, agg.VerifyConclusion) ("张三", "王五", true, true) "聚合值有误"
          let com = ApproveAccount(ApprovedBy = "赵六", Approved = false, Limit = 10m)
          let agg = svc.ApproveAccount None id2 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          let com = LimitAccount(Limit = 20m)
          let f = fun _ -> svc.LimitAccount None id2 com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "通过审核，但未批准"
    |> testSequenced
    |> testLabel "Account"
...
let test4 =
    [ testCase "初始化"
      <| fun _ ->
          let com = CreateAccount(Owner = "张三")
          let agg = svc.CreateAccount None id3 com |> Async.RunSynchronously
          Expect.equal agg.Owner "张三" "聚合值有误"
          let com = VerifyAccount(VerifiedBy = "王五", Conclusion = false)
          let agg = svc.VerifyAccount None id3 com |> Async.RunSynchronously
          Expect.equal (agg.Owner, agg.VerifiedBy, agg.Verified, agg.VerifyConclusion) ("张三", "王五", true, false) "聚合值有误"
      testCase "提交审批命令"
      <| fun _ ->
          let com = ApproveAccount(ApprovedBy = "赵六", Approved = true, Limit = 10m)
          let f = fun _ -> svc.ApproveAccount None id3 com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "提交设置限额命令"
      <| fun _ ->
          let com = LimitAccount(Limit = 20m)
          let f = fun _ -> svc.LimitAccount None id3 com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "未通过审核"
    |> testSequenced
    |> testLabel "Account"
