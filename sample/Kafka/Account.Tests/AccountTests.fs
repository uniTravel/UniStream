...
let test1 =
    [ testCase "初始化"
      <| fun _ ->
          ...
          id <- Guid.NewGuid()
          let agg = svc.CreateAccount None id com |> Async.RunSynchronously
          Expect.equal agg.Owner "张三" "聚合值有误"
          let com = VerifyAccount(VerifiedBy = "王五", Conclusion = false)
          let agg = svc.VerifyAccount None id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.ApproveAccount None id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "提交设置限额命令"
      <| fun _ ->
          let com = LimitAccount(Limit = 20m)
          let f = fun _ -> svc.LimitAccount None id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "未通过审核"
    |> testSequenced
    |> testLabel "Account"
...
let test2 =
    [ testCase "初始化"
      <| fun _ ->
          ...
          let agg = svc.ApproveAccount None id com |> Async.RunSynchronously
...
let test3 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          let com = LimitAccount(Limit = 20m)
          let agg = svc.LimitAccount None id com |> Async.RunSynchronously
