...
let test1 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          ...
          let agg = svc.TransferOut trace id com |> Async.RunSynchronously
          Expect.equal (agg.AccountId, agg.Limit, agg.TransLimit, agg.Balance) (acid, 1000m, 1000m, 94m) "聚合值有误"
      testCase "设置的交易限额超过账户限额"
      <| fun _ ->
          let com = { TransLimit = 1050m }
          let f = fun _ -> svc.SetTransLimit trace id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "设置交易限额"
      <| fun _ ->
          let com = { TransLimit = 500m }
          let agg = svc.SetTransLimit trace id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = svc.ChangeLimit trace id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.Withdraw trace id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "转出金额超过余额"
      <| fun _ ->
          let com: TransferOut = { Amount = 100m }
          let f = fun _ -> svc.TransferOut trace id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误" ]
    |> testList "初始创建的交易期间"
    |> testSequenced
...
let test2 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          let com = { Amount = 5m }
          let f = fun _ -> svc.Deposit trace id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      ...
      <| fun _ ->
          ...
          let agg = svc.SetLimit trace id com |> Async.RunSynchronously
          Expect.equal (agg.AccountId, agg.Limit, agg.TransLimit, agg.Balance) (acid, 1000m, 1000m, 94m) "聚合值有误"
      testCase "再次执行存款"
      <| fun _ ->
          let com = { Amount = 5m }
