...
let test1 =
    [ testCase "初始化"
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.SetTransLimit id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "设置交易限额"
      <| fun _ ->
          let com = { TransLimit = 500m }
          let agg = svc.SetTransLimit id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = svc.ChangeLimit id com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let f = fun _ -> svc.Withdraw id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      testCase "转出金额超过余额"
      <| fun _ ->
...
let test2 =
    [ testCase "初始化"
      <| fun _ ->
          ...
          let agg = svc.OpenPeriod (Guid.NewGuid()) com |> Async.RunSynchronously
          id <- agg.Id
          Expect.equal (agg.AccountId, agg.Limit, agg.TransLimit, agg.Balance) (acid, 0m, 0m, 0m) "聚合值有误"
      testCase "打开但未生效的交易期间执行存款命令"
      <| fun _ ->
          let com = { Amount = 5m }
          let f = fun _ -> svc.Deposit id com |> Async.RunSynchronously |> ignore
          Expect.throwsT<ValidateError> f "异常类型有误"
      ...
      <| fun _ ->
          ...
          let agg = svc.SetLimit id com |> Async.RunSynchronously
          Expect.equal (agg.AccountId, agg.Limit, agg.TransLimit, agg.Balance) (acid, 1000m, 1000m, 94m) "聚合值有误"
      testCase "再次执行存款"
      <| fun _ ->
          let com = { Amount = 5m }
