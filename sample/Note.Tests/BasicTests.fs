...
let tests =
    ...
    testSequenced <| testList "可变聚合基本处理" [
        testCase "给定三个聚合ID，创建三个Note" <| fun _ ->
            ...
            let r1 = app.CreateNote aggId1 (Guid.NewGuid().ToString()) c1 |> Async.RunSynchronously
            let r2 = app.CreateNote aggId2 (Guid.NewGuid().ToString()) c2 |> Async.RunSynchronously
            let r3 = app.CreateNote aggId3 (Guid.NewGuid().ToString()) c3 |> Async.RunSynchronously
        ...
        testCase "修改第一个Note" <| fun _ ->
            ...
            let r1 = app.ChangeNote aggId1 (Guid.NewGuid().ToString()) c1 |> Async.RunSynchronously
            Expect.equal r1.Content c1.Content "返回值错误。"
        testCase "连续修改第二个Note" <| fun _ ->
            let c2 = { Content = "changed content" }
            let r2 = app.ChangeNote aggId2 (Guid.NewGuid().ToString()) c2 |> Async.RunSynchronously
            Expect.equal r2.Content c2.Content "返回值错误。"
