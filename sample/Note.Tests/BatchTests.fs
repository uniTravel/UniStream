...
let tests =
    ...
    testSequenced <| testList "可变聚合批处理" [
        testCase "给定聚合ID，创建Note" <| fun _ ->
            ...
            let r = app.BatchCreate "test" aggId createNote (Guid.NewGuid().ToString()) c |> Async.RunSynchronously
            Expect.equal r.Content c.Content "返回值错误。"
        testCase "连续修改Note" <| fun _ ->
            let r =
                seq { 1 .. 5000 }
                |> Seq.map (fun i -> app.BatchChange "test" aggId changeNote (Guid.NewGuid().ToString()) { Content = "changed content" })
                |> Async.Parallel
