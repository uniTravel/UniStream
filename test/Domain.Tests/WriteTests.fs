...
let test =
    [ testCase "创建两个除Id外其他值相等的聚合"
      <| fun _ ->
          ...
          let agg = create agent id1 com |> Async.RunSynchronously
          Expect.equal (agg.Revision, agg.Title, agg.Content, agg.Grade) (0UL, "t", "c", 1) "聚合值有误"
          let agg = create agent id2 com |> Async.RunSynchronously
          Expect.equal (agg.Revision, agg.Title, agg.Content, agg.Grade) (0UL, "t", "c", 1) "聚合值有误"
      ...
      <| fun _ ->
          ...
          let f = fun _ -> create agent id3 com |> Async.RunSynchronously |> ignore
      ...
      <| async {
          ...
          let! r = coms |> List.map (fun c -> create agent (Guid.NewGuid()) c) |> Async.Parallel
