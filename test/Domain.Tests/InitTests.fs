...
let test =
    [ testCase "创建聚合"
      ...
      <| fun _ ->
          ...
          let chg = { Content = "c1" }
          let f = fun _ -> change agent None id chg |> Async.RunSynchronously |> ignore
      ...
      <| fun _ ->
          ...
          let chg = { Content = "c1" }
          let agg = change agent None id chg |> Async.RunSynchronously
          Expect.equal (agg.Id, agg.Revision, agg.Title, agg.Content, agg.Grade) (id, 1UL, "t", "c1", 1) "聚合值有误"
      testCase "应用第二条未注册重播的变更"
      <| fun _ ->
          let chg = { Up = 1 }
          let agg = upgrade agent None id chg |> Async.RunSynchronously
