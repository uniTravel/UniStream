...
let test =
    [ testCase "创建第一个聚合"
      <| fun _ ->
          ...
          let agg = create agent i1 com |> Async.RunSynchronously
      ...
      <| fun _ ->
          ...
          let agg = create agent i2 com |> Async.RunSynchronously
          Expect.equal (agg.Revision, agg.Title, agg.Content, agg.Grade) (0UL, "t", "c", 1) "聚合值有误"
      testCase "第二个聚合持续应用八条变更，以便触发缩容"
      <| fun _ ->
          let c = [ for i in 1..8 -> { Content = $"c{i}" } ]
          let r = c |> List.map (fun c -> change agent i2 c |> Async.RunSynchronously)
          let agg = r[7]
          Expect.equal (agg.Revision, agg.Title, agg.Content, agg.Grade) (8UL, "t", "c8", 1) "聚合值有误"
      testCase "第一个聚合应用第一条变更"
      <| fun _ ->
          let com = { Content = "c1" }
          let f = fun _ -> change agent i1 com |> Async.RunSynchronously |> ignore
